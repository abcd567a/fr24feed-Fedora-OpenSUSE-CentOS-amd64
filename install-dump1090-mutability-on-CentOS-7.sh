...
##sudo make
### Note: Above `make` command fails due to old versions of `make` and `gcc` in CentOS-7 repository.
### We will therefore download pre-build binary from Github

echo -e "\e[01;32mDownloading dump1090-mutability linux binary from Github \e[0;39m"
sudo wget -O ${ASSETS_FOLDER}/dump1090-mutability-1.15 "https://github.com/abcd567a/fr24feed-Fedora-OpenSUSE-CentOS-amd64/releases/download/v1.0/dump1090-mutability-1.15"
echo -e "\e[01;32mMaking dump1090-mutability linux binary executeable \e[0;39m"
sudo chmod +x ${ASSETS_FOLDER}/dump1090-mutability-1.15
echo -e "\e[01;32mCopying Executeable Binary to folder `/usr/bin/` \e[0;39m"
sudo cp ${ASSETS_FOLDER}/dump1090-mutability-1.15 /usr/bin/dump1090-mutability
